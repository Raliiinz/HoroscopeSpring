package ru.itis.horoscope.library;
...
/**
 * Крошечная библиотека Java для расчета положения и фаз солнца и луны
 * Created with  IDEA.
 *
...
public class SunCalc4JavaUtils {
    ...
    /*Вычисляется юлианская дата (количество дней с эпохи J1970):
    date.getTime() — количество миллисекунд с 1 января 1970 года.
    dayMs — число миллисекунд в одном дне.
    J1970 — юлианская дата для 1 января 1970 года.
     */
    ...
    //Где J2000 = 2451545 — юлианская дата для 1 января 2000 года.
     ...
     * @param d
     * @return
     * Этот метод moonCoords(double d) вычисляет координаты Луны в пространстве и расстояние до нее. Вот подробное объяснение:
     *
     * Входные данные:
     * d - количество дней с начала эпохи J2000
     * Вычисляемые значения:
     * Elongation (L): Вычисляется как 218.316 + 13.176396 * d Представляет долготу эклиптической орбиты Луны
     * Средняя аномалия (M):
     * Вычисляется как 134.963 + 13.064993 * d Представляет угол между точкой пересечения Луны с эклиптикой и ее ближайшей точке к Земле
     * Среднее расстояние (F):
     * Вычисляется как 93.272 + 13.229350 * d
     * Представляет среднее расстояние Луны от Земли в радиусах Земли
     * Долгота (l):
     * Вычисляется как L + 6.289 * sin(M)
     * Представляет гецентрическую долготу Луны
     * Широта (b):
     * Вычисляется как 5.128 * sin(F)
     * Представляет гецентрическую широту Луны
     * Расстояние до Луны (dt):
     * Вычисляется как 385001 - 20905 * cos(M)
     * Представляет расстояние в километрах от центра Земли до Луны
     * Результаты:
     * Метод возвращает Map<String, Double> с тремя ключами:
     *
     * "ra": правое восхождение (right ascension) Луны
     * "dec": деклинация Луны
     * "dist": расстояние до Луны в километрах
     * Используемые вспомогательные методы:
     * rightAscension(l, b) и declination(l, b) - вероятно, методы для вычисления астрономических координат из долготы и широты
     */
    public static Map<String, Double> moonCoords(double d) {
        ...
        // mean anomaly
        //Это значение определяет, насколько Луна отклонилась от своей эллиптической орбиты вокруг Земли.
